...
\dfn{Discharging a Capacitor}{
  The capacitor discharges through the resistor. \\
  $$ \frac{q}{c} + iR = 0 \to \frac{q}{c} = - iR \to - \frac{q}{RC} = i $$ 
  $$ i = \frac{dq}{dt} = - \frac{q}{RC} $$ 
  $$ \frac{dq}{q} = - \frac{dt}{RC} $$
  $$ \int_{Q_\infty}^{Q} \frac{dq}{q} = - \int_{0}^{t} \frac{dt}{RC} $$
  $$ \ln(q) |^Q_{Q_0} = - \frac{t}{RC} $$
  $$ \ln \left(\frac{Q}{Q_0} \right) = - \frac{t}{RC} $$
  $$ \frac{Q}{Q_0} = e^{- \frac{t}{RC}} $$ 
  $$ Q = Q_0 e^{- \frac{t}{RC}} $$ 
}

\subsubsection{Charge, Current, and Voltage in a Discharging Capacitor}
\dfn{Change in C, A, and V}{
  $$ q = Q_0 e^{- \frac{t}{RC}} $$ 
  $$ I = - \frac{Q_0}{RC} e^{- \frac{t}{RC}} = I_{0}e^{-\frac{t}{RC}}$$ 
  $$ V = - Q_0 e^{- \frac{t}{RC}} $$
}
...
\dfn{Magnetic Fields}{
  \textbf{Properties:}
  \begin{itemize}
    \item Magnetic fields are created by moving charges. 
    \item Magnetic fields are vectors. 
    \item Magnetic fields are measured in Tesla(T) or Gauss(G). 
    \item Magnetic fields are created by moving charges. 
  \end{itemize}
}

\subsection{Magnetic Force on a Moving Charge}
\dfn{Magnetic Force on a Moving Charge}{
  The force on a moving charge in a magnetic field is perpendicular to both the velocity of the charge and the magnetic field. \\
  $$ F = q\vec{v}x\vec{B} $$ 
  $$ F = qvB $$ 
  $$ F = qvB\sin(90) = qvB $$ 
  $$ F = qvB\sin(180) = 0 $$ 
...
\subsection{Trajectories of Charged Particles}
\dfn{Trajectories of Charged Particles}{
  When moving in a uniform field
}

\subsection{Velocity Selector}

\subsection{Lorentz Force}
\dfn{Lorentz Force Law}{
  The force on a charge moving in both an electric and magnetic field. \\
  $$ \vec{F} = q\vec{E} + q\vec{v} x \vec{B} $$ 
}

\subsection{Hall Effect}
\dfn{Charges in a conductor - The Hall Effect}{
  \begin{enumerate}
    \item A current is introduced through a conductor, and a magnetic field is applied perpendicular to the current.
    \item Charge carriers deflected. The direction of the deflection independent on the charge. 
    \item As charges deflect, an electric field is created acrosss the conductor, and the direction of the electric field is dependent on the charge of the charge carriers. The elctric field is perpendicular to both the current and the magnetic field. 
    \item Charges are deflected until $ F_{\text{magnetic}} = F_{\text{electric}} \to  F_e = F_b$
    \item The "Hall Voltage" can be measured to determine the sign and magnitude of the density of the charge carriers in the conductor.
  \end{enumerate}
  $$ V_H = Ed $$ 
  $$ qE = qv_dB $$ 
  $$ v_d = \frac{J}{ne} = \frac{i}{neA} $$ 
  $$ V_H = dv_dB $$ 
  $$ \text{Charge Density: } n = \frac{Bi}{V_H d e} $$ 
}

\subsection{Compass Needles}

\dfn{Compass Needles}{
  Compass needles align with the magnetic field. 
}

\section{Magnetic Materials}
\dfn{Why are some materials magnetic?}{
  \begin{itemize}
    \item Magnetic materials have magnetic domains. 
    \item Magnetic domains are regions of aligned magnetic fields. 
    \item When a magnetic field is applied, the domains align. 
    \item When the magnetic field is removed, the domains remain aligned. 
  \end{itemize}
}

\qs{Calculate B}{
  A particle of charge q and mass m is accelertaed from rest by an electric field E through a distance d and enters and exists a region containing a constant magnetic field B at points shown. \\ Assume q, m, E, d, and $x_0$ are known. \\ 
  The particle exits the magnetic field at $x = x_0/2$ and $y = x_0/2$. \\
  First we find the velocity of the particle when it enters the magnetic field. \\
  $$ U = qV = qEd = \frac{1}{2} mv_0^2  \to v_0 = \sqrt{\frac{2qED}{m}} $$
  Now, we need to find the radius of the particle's trajectory in the magnetic field. \\
  $$ R = \frac{1}{2} x_0 $$ 
  $$ F= m \frac{v_0^2}{R} = qv_0B $$ 
  $$ B = m\frac{v_0^2}{R v_0 q } = m \frac{v_0}{R q} = \frac{m}{Rq} \sqrt{\frac{2qED}{m}} = \frac{2}{x_0}$$
}

\section{Magnetic Force \& Torque on Currents}

\subsection{Magnetic Force on a Current-Carrying Wire}
\dfn{Magnetic Force on a Current-Carrying Wire}{
...
\dfn{Torque on a Current Loop}{
  The torque on a current loop in a magnetic field is perpendicular to both the current and the magnetic field. \\
  $$ \vec{\tau} = \vec{R} x \vec{F} \text{ where R is the distance from the axis of rotation}$$ 
  $$ \vec{\tau} = \vec{R}x\vec{F} = \vec{R}xI\vec{L}x\vec{B} =  I\vec{A}x\vec{B} $$
  $$ \vec{\tau} = I\vec{A}x\vec{B} $$ 
  $$ \vec{\tau} = IAB\sin(90) = IAB $$ 
  $$ \vec{\tau} = IAB\sin(180) = 0 $$ 
}
\nt{
  How to find the direction of the torque: \\
  It should be up the axis of rotation if the current is going counter-clockwise. \\
  It should be down the axis of rotation if the current is going clockwise.
}

\subsection{Magnetic Dipole Moment}
\dfn{Magnetic Dipole Moment}{
  $$ \mu \equiv NI\vec{A} $$
  $$ \vec{\tau} = NI\vec{A}x\vec{B} = \vec{\mu}x\vec{B} $$
  $$ \vec{\tau} = \vec{\mu} x \vec{B} $$
}
\nt{
  $\mu $ makes torque easy! \\
  The torque always wants to line $ \mu $ up with $B$. It turns towards $B$. \\
  Torque is greatest when $ \mu $ is perpendicular to $B$.
}

\subsection{Work Done By a Magnetic Field on a Current Loop}
\dfn{Work Done By a Magnetic Field on a Current Loop}{
  $$ W = \int \tau d\theta $$ 
  $$ \vec{\tau} = \vec{\mu} x \vec{B} = \mu B \sin(\theta) $$ 
  $$ W = \int \mu B \sin(\theta) d\theta = - \mu \cdot B \cos(\theta) |^{\theta_2}_{\theta_1} = \vec{\mu} \cdot \vec{B} (\cos(\theta_1) - \cos(\theta_2)) $$
  $$ \Delta U = -W $$ 
  $$ U \equiv = - \vec{\mu} \cdot \vec{B} $$
}

\section{Finding the Magnetic Field}

\subsection{Magnetic Constant}
\dfn{Magnetic Constant}{
  $$ \mu_0 = 4\pi \cdot 10^{-7} Tm/A $$
  $$ K' = \frac{\mu_0}{4\pi} $$ 
}

\subsection{Boit-Savant Law}
\dfn{Boit-Savant Law}{
  The fundamental law for determining the direction and magnitude of the magnetic field due to an element of current. \\
  $$ dB = \frac{\mu_0I}{4\pi} \frac{d\vec{s} x \hat{r}}{r^2} $$
  Infinite straight wire: $$ B = \frac{\mu_0I}{2\pi r} $$ 
  Circular loop: $$ B = \frac{\mu_0I R^2}{2(R^2 + z^2)^{\frac{3}{2}}} $$
}

\qs{Derive the magnetic field from a long wire}{
  $$ dB = \frac{\mu_0I}{4\pi} \frac{d\vec{x} x \hat{r}}{r^2} \to dB = \frac{u_0 I}{4\pi } \frac{dx}{r^2} \cos \theta $$
  $$ x = R \tan \theta \to dx = R \sec^2 \theta d\theta $$ 
  $$ r = \frac{R}{\cos \theta } $$
  $$ B = \int dB $$ 
  $$ B = \frac{\mu I}{4\pi R} \ldots $$ 
}

\qs{Are two parallel wires with currents in the same direction attracted to each other?}{
  Yes, because the magnetic field of one wire is in the same direction as the magnetic field of the other wire. EDIT
}

\subsection{Ampere's Law}
\dfn{Ampere's Law}{
  Ampere's Law is a general law that relates the magnetic field around a closed loop to the current passing through the loop. \\
  $$ \oint \vec{B} \cdot d\vec{l} = \mu_0 I_{\text{enclosed}} $$
  $$ B = \frac{\mu_0 I}{2\pi r} $$
}
\nt{
  \textbf{Amperian loop goals:} 
  \begin{itemize}
    \item $ B \cdot dl = Bdl $ 
    \item Everything is perpendicular to the loop. 
  \end{itemize}
}

\qs{Find the magnetic field of a concentric wire}{
}
...
\qs{Find the magnetic field $ z $ above a 2d infinite sheet of current}{
  $$ \oint \vec{B} \cdot d\vec{l} = \mu_0 I_{\text{enclosed}} = \mu_0 (L \cdot J) $$
  $$ \oint \vec{B} d\vec{l} = \mu_0 I_{\text{enclosed}} = \int _{1\to 2} \vec{B} d\vec{l} + \int_{2\to z} \vec{B} d\vec{l} + \int _{2\to 1} \ldots + \int _{1 \to -z} \ldots  $$ 
  $$ BL + 0 + BL + 0 = 2BL \text{ (L is the length of the amperian loop)} $$ 
  $$ B = \frac{\mu_0J}{2} $$ 
  Magnetic field doesn't depend on distance from the sheet for an infinite sheet of current. Or a sheet of current with a length much greater than the distance from the sheet.
}

\qs{Find the magnetic field $z$ above a 3d infinite sheet of current}{
  $$ \oint \vec{B} \cdot d\vec{l} = \mu _0 I_{\text{enclosed}} = \mu_0 J \cdot A $$
  $$ \oint \vec{B} \cdot d\vec{l} = \mu_0 J \cdot L \cdot 2z $$
  $$ B = \mu_0 Jz $$ 
  $$ B = 
    \begin{dcases}
        -\mu_0 J z \cdot \hat{y}  & 0 < z < \frac{t}{2} \\
        \mu_0 J z \cdot \hat{y} &  0 > z > -\frac{t}{2} \\    
    \end{dcases}
  $$
}
...
\nt{
  ...
  \begin{itemize}
    \item Magnetic forces may be exerted on the charge carriers in the conductor. 
    \item These forces produce a charge seperation in the conductor. 
    \item This charge dstribution creates an electric field in the conductor. 
    \item The equilibrium distribution is reached when the forces from the electric and magnetic field cancel. 
    \item The equilibrium electric field produces a potential difference (motional emf) in the conductor. 
  \end{itemize}
}
\nt{
  $$ qvB = qE \to E = vB $$
  ...
  $$ \mathcal{E} = vBL $$
  $$ F = I \vec{L} x \vec{B} $$ 
  $$ F = \frac{vBL}{R} LB \to P = Fv = \frac{vBL}{R} LBv = I^2R $$
}

\section{Magnetic Flux}
\dfn{Flux}{
  The rate of flow through a surface.
  $$ \Phi = \int \vec{B} d\vec{A} = \vec{B}\vec{A}\cos \theta  $$
  Magnetic Flux Units: Weber(Wb) = Tesla $\cdot$ m$^2$
}

\section{Faraday's Law}
\dfn{Faraday's Law}{
  The induced emf in a circuit is equal to the rate of change of magnetic flux through the circuit. One of Maxwell's equations. \\
  $$ \mathcal{E} = - N \frac{d\Phi}{dt} $$
  $$ \mathcal{E} = \oint \vec{E} \cdot d\vec{l} = - \frac{d\Phi}{dt} $$
  \\
  The changing magnetic flux produces an electric field. \\
  Electricity and magnetism are two sides of the same coin.
}

\section{Lenz's Law}
\dfn{Lenz's Law}{
  When the flux $ \Phi $ through a loop changes, an emf is induced in the loop. \\
  The emf will make a current flow if it can (like a battery). \\
  The current that flows induces a new magnetic field. \\ \\
  The new magnetic field opposes the change in the original magnetic field that created it. (Lenz's Law). The direction of the induced emf is such that it opposes the change in magnetic flux that produced it. \\ \\
  \textbf{Example:} If the magnetic field is increasing, the induced emf will create a magnetic field that opposes the increase. 
}
\section{Inductor Circuits}
\subsection{LR Circuits}
\dfn{LR Time Constant}{
  The time it takes for the current in an LR circuit to reach 63.2\% of its final value. \\
  $$ \tau = \frac{L}{R} $$
  \\
  \textbf{Current when voltage supply is connected:} \\
  $$ I = \frac{\mathcal{E}}{R} (1 - e^{-\frac{t}{\tau}}) $$ 
  \\
  \textbf{Current when voltage supply is disconnected:} \\
  $$ I = I_0 e^{-\frac{t}{\tau}} $$
}
...
\dfn{Maxwell's Equations}{
  \begin{enumerate}
    \item Gauss's Law for Electricity: $$ \oint \vec{E} \cdot d\vec{A} = \frac{Q_{\text{enclosed}}}{\varepsilon_0} $$ 
    \item Gauss's Law for Magnetism: $$ \oint \vec{B} \cdot d\vec{A} = 0 $$ 
    \item Faraday's Law: $$ \oint \vec{E} \cdot d\vec{l} = - \frac{d\Phi}{dt} $$ 
    \item Ampere's Law/Maxwell's Law of Induction: $$ \oint \vec{B} \cdot d\vec{l} = \mu_0 I_{\text{enclosed}} + \mu_0 \varepsilon_0 \frac{d\Phi}{dt} = \mu _0 I_{\text{enc}} + \mu_0 \varepsilon_0 \frac{d}{dt} \oint \vec{E} \cdot \vec{dA} $$
  \end{enumerate}
}
